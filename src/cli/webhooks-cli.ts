...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.starforge.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + StarforgeOS hooks")
    ...
    .option("--hook-url <url>", "StarforgeOS hook URL")
    .option("--hook-token <token>", "StarforgeOS hook token")
