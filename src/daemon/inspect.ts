...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "starforge" | "clawdbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["starforge", "clawdbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["starforge_service_marker"];
  const kindKeys = ["starforge_service_kind"];
...
function isOpenClawGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.starforge.");
}
...
function isOpenClawGatewaySystemdService(name: string, contents: string): boolean {
  ...
  if (!name.startsWith("starforge-gateway")) {
    return false;
  }
...
function isOpenClawGatewayTaskName(name: string): boolean {
  ...
  return normalized === defaultName || normalized.startsWith("starforge gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "starforge" && isOpenClawGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "starforge" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "starforge" && isOpenClawGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
