...
- `/usage cost` in chats: local cost summary aggregated from StarforgeOS session logs.
- CLI: `starforge status --usage` prints a full per-provider breakdown.
- CLI: `starforge channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
