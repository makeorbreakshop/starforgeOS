...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.STARFORGEOS_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.STARFORGEOS_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.STARFORGEOS_NODE_OPTIONS_READY = "1";
  ...
  child.once("error", (error) => {
    console.error(
      "[starforge] Failed to respawn CLI:",
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[starforge] ${parsed.error}`);
    process.exit(2);
  }
