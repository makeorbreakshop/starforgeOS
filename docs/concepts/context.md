...
read_when:
  - You want to understand what “context” means in StarforgeOS
...
“Context” is **everything StarforgeOS sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (StarforgeOS-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How StarforgeOS builds the system prompt

The system prompt is **StarforgeOS-owned** and rebuilt each run. It includes:
...
By default, StarforgeOS injects a fixed set of workspace files (if present):
