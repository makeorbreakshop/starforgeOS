...
```bash
starforge plugins install @openclaw/msteams
```

Local checkout (when running from a git repo):

```bash
starforge plugins install ./extensions/msteams
```
...
3. Configure StarforgeOS with those credentials.
...
- Talk to StarforgeOS via Teams DMs, group chats, or channels.
...
- On startup, StarforgeOS resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.starforgeos/starforgeos.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring StarforgeOS, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                       |
   | ------------------ | ----------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `starforgeos-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `starforge plugins install @openclaw/msteams`
   - From a local checkout: `starforge plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "StarforgeOS in Teams", "full": "StarforgeOS in Teams" },
...
StarforgeOS handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, StarforgeOS only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
StarforgeOS sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `starforge message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.starforgeos/msteams-polls.json`.
...
```bash
starforge message send --channel msteams \
...
starforge message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
starforge message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
starforge message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
starforge message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
