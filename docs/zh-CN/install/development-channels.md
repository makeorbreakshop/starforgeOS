...
当你使用 `--channel` **显式**切换渠道时，StarforgeOS 还会对齐安装方式：

- `dev` 确保有一个 git checkout（默认 `~/starforgeos`，可通过 `STARFORGEOS_GIT_DIR` 覆盖），
...
当你使用 `starforge update` 切换渠道时，StarforgeOS 还会同步插件来源：
