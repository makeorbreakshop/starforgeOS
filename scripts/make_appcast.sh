...
ZIP=${1:?"Usage: $0 StarforgeOS-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/starforgeos/starforgeos/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^StarforgeOS-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/makeorbreakshop/starforgeOS/releases/download/v${VERSION}/"}
