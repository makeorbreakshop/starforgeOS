...
import { resolveOpenClawPackageRoot } from "../infra/starforge-root.js";
import {
  channelToNpmTag,
  DEFAULT_GIT_CHANNEL,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "starforge doctor entry": "Checking doctor entrypoint",
  "starforge doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "starforge";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const STARFORGEOS_REPO_URL = "https://github.com/starforge/starforge.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), ".starforge");
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("starforge@")) {
    return trimmed.slice("starforge@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "starforge.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("starforge completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.STARFORGEOS_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", STARFORGEOS_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `STARFORGEOS_GIT_DIR points at a non-git directory: ${params.dir}. Set STARFORGEOS_GIT_DIR to an empty folder or an starforge checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`STARFORGEOS_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("StarforgeOS update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating StarforgeOS..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this StarforgeOS install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("starforge doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("starforge gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g starforge@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g starforge@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("starforge gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("starforge doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("starforge gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("starforge gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `starforge update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `STARFORGEOS_GIT_DIR points at a non-git directory: ${gitDir}. Set STARFORGEOS_GIT_DIR to an empty folder or an starforge checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via STARFORGEOS_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update StarforgeOS to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["starforge update", "Update a source checkout (git)"],
        ["starforge update --channel beta", "Switch to beta channel (git + npm)"],
        ["starforge update --channel dev", "Switch to dev channel (git + npm)"],
        ["starforge update --tag beta", "One-off update to a dist-tag or version"],
        ["starforge update --no-restart", "Update without restarting the service"],
        ["starforge update --json", "Output result as JSON"],
        ["starforge update --yes", "Non-interactive (accept downgrade prompts)"],
        ["starforge update wizard", "Interactive update wizard"],
        ["starforge --update", "Shorthand for starforge update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run starforge update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.starforge.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["starforge update status", "Show channel + version status."],
          ["starforge update status --json", "JSON output."],
          ["starforge update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.starforge.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
