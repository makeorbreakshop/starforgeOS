...
export async function statusCommand(
  ...
  runtime.log(theme.heading("StarforgeOS status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("starforge security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("starforge security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.starforge.ai/faq");
  runtime.log("Troubleshooting: https://docs.starforge.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("starforge status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("starforge logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("starforge status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("starforge gateway probe")}`);
  }
}
