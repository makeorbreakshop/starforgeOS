...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "starforge-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "starforge.mjs");
    ...
    const doctorStep = await runStep(
      step("starforge doctor", doctorArgv, gitRoot, { STARFORGEOS_UPDATE_IN_PROGRESS: "1" }),
    );
