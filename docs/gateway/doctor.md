...
`starforge doctor` is the repair + migration tool for StarforgeOS. It fixes stale
...
```bash
starforge doctor --yes
```

Accept defaults without prompting (including restart/service/sandbox repair steps when applicable).

```bash
starforge doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
starforge doctor --repair --force
```
...
```bash
cat ~/.starforgeos/starforgeos.json
```
...
- Extra workspace dir detection (`~/starforgeos`).
...
you to run `starforge doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.starforgeos/starforgeos.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.starforgeos/sessions/` to `~/.starforgeos/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.starforgeos/agent/` to `~/.starforgeos/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.starforgeos/credentials/*.json` (except `oauth.json`)
  - to `~/.starforgeos/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `starforge doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.starforgeos` folders exist across
  home directories or when `STARFORGEOS_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.starforgeos/starforgeos.json` is
...
offers to remove them and install the StarforgeOS service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named StarforgeOS gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `starforge doctor --generate-gateway-token` to force token
...
- `starforge doctor` prompts before rewriting supervisor config.
- `starforge doctor --yes` accepts the default repair prompts.
- `starforge doctor --repair` applies recommended fixes without prompts.
- `starforge doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `starforge gateway install --force`.
