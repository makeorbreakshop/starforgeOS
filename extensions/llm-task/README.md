...
`starforgeos.invoke --each`) without adding new StarforgeOS code per workflow.
...
This extension depends on StarforgeOS internal modules (the embedded agent runner).
It is intended to ship as a **bundled** StarforgeOS extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.starforgeos/extensions` as a standalone plugin directory.
