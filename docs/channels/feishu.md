...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects StarforgeOS to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
starforge plugins install @openclaw/feishu
```

Local checkout (when running from a git repo):

```bash
starforge plugins install ./extensions/feishu
```
...
If you just installed StarforgeOS, run the wizard:
...
2. Configuring app credentials in StarforgeOS
...
1. You already ran `starforge channels add` for Feishu
2. The gateway is running (`starforge gateway status`)
...
Edit `~/.starforgeos/starforgeos.json`:
...
```bash
starforge pairing approve feishu <CODE>
```
...
2. Run `starforge logs --follow` and look for `chat_id`
...
2. Run `starforge logs --follow` and look for `open_id`
...
| Command                     | Description                   |
| --------------------------- | ----------------------------- |
| `starforge gateway status`  | Show gateway status           |
| `starforge gateway install` | Install/start gateway service |
| `starforge gateway stop`    | Stop gateway service          |
| `starforge gateway restart` | Restart gateway service       |
| `starforge logs --follow`   | Tail gateway logs             |
...
4. Check logs: `starforge logs --follow`
...
5. Ensure the gateway is running: `starforge gateway status`
6. Check logs: `starforge logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.starforgeos/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.starforgeos/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
