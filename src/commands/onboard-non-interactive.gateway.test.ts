...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "starforge-onboard-"));
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "starforge");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.STARFORGEOS_STATE_DIR = stateDir;
    process.env.STARFORGEOS_CONFIG_PATH = path.join(stateDir, "starforge.json");
