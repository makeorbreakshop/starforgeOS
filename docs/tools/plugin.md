---
summary: "StarforgeOS plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends StarforgeOS with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core StarforgeOS yet (or you want to keep optional features out of your main
install).
...
StarforgeOS plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
If a plugin exports `id`, StarforgeOS uses it but warns when it doesn’t match the
configured id.
...
StarforgeOS augments `uiHints` at runtime based on discovered plugins:
...
Plugins may also register their own top‑level commands (example: `starforge voicecall`).
...
- Plugin-managed hooks show up in `starforge hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `starforge hooks`; enable/disable the plugin instead.

## Provider plugins (model auth)

Plugins can register **model provider auth** flows so users can run OAuth or
API-key setup inside StarforgeOS (no external scripts needed).
...
- `config`: The current StarforgeOS config
