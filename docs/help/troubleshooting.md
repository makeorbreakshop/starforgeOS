...
You can also set `STARFORGEOS_VERBOSE=1` instead of the flag.
...
### `docs.starforgeos.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.starforgeos.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.starforgeos.ai` to the allowlist, then retry.
...
- Check the allowlist: `starforge config get agents.defaults.models`
...
If you can, include the relevant log tail from `starforge logs --follow`.
