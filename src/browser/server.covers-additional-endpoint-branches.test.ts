  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "starforge",
        profiles: {
          starforge: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control server", () => {
  beforeEach(async () => {
    ...
    process.env.STARFORGEOS_GATEWAY_PORT = String(testPort - 2);
...
describe("backward compatibility (profile parameter)", () => {
  ...
  it("GET / without profile uses default profile", async () => {
    ...
    // Should use default profile (starforge)
    expect(status.profile).toBe("starforge");
  });
  ...
  it("POST /start without profile uses default profile", async () => {
    ...
    expect(result.profile).toBe("starforge");
  });
  ...
  it("GET /profiles returns list of profiles", async () => {
    ...
    // Should at least have the default starforge profile
    expect(result.profiles.some((p) => p.name === "starforge")).toBe(true);
  });

  it("GET /tabs?profile=starforge returns tabs for specified profile", async () => {
    const { startBrowserControlServerFromConfig } = await import("./server.js");
    await startBrowserControlServerFromConfig();
    const base = `http://127.0.0.1:${testPort}`;
    ...
    const result = (await realFetch(`${base}/tabs?profile=starforge`).then((r) => r.json())) as {
      running: boolean;
      tabs: unknown[];
    };
    expect(result.running).toBe(true);
  ...
  it("POST /tabs/open?profile=starforge opens tab in specified profile", async () => {
    const { startBrowserControlServerFromConfig } = await import("./server.js");
    await startBrowserControlServerFromConfig();
    const base = `http://127.0.0.1:${testPort}`;
    ...
    const result = (await realFetch(`${base}/tabs/open?profile=starforge`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ url: "https://example.com" }),
    }).then((r) => r.json())) as { targetId?: string };
