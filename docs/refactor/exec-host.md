...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.starforgeos/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.starforgeos/node.json` (node id + pairing token).
...
Path: `~/.starforgeos/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.starforgeos/exec-approvals.sock",
...
- Unix socket at `~/.starforgeos/exec-approvals.sock` (0600).
