...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in StarforgeOS
...
Running StarforgeOS via Node with `tsx` fails at startup with:

```
[starforgeos] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `starforge status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch starforgeos.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node starforgeos.mjs status` works on Node 25.
