---
summary: "Use MiniMax M2.1 in StarforgeOS"
read_when:
  - You want MiniMax models in StarforgeOS
...
```bash
starforge plugins enable minimax-portal-auth  # skip if already loaded.
starforge gateway restart  # restart if gateway is already running
starforge onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/makeorbreakshop/starforgeOS/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `starforgeos.json`:
...
- Use `starforge models list` and `starforge models set minimax/MiniMax-M2.1` to switch.
...
- Running `starforge configure` and selecting **MiniMax M2.1**, or
