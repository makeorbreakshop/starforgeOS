...
- **gateway host** → `starforgeos` process on the gateway machine
...
`~/.starforgeos/exec-approvals.json` directly.

CLI: `starforge approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
