...
```bash
starforge plugins install @openclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
starforge plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "StarforgeOS" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `starforge pairing list nextcloud-talk`
  - `starforge pairing approve nextcloud-talk <CODE>`
