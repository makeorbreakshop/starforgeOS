...
StarforgeOS features that can generate provider usage or paid API calls.
...
- `starforge status --usage` and `starforge channels list` show provider **usage windows**
...
StarforgeOS can pick up credentials from:
...
- `starforge status --usage`
- `starforge models status --json`
...
`starforge models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
