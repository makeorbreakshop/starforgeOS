...
INSTALL_URL="${STARFORGEOS_INSTALL_URL:-https://starforgeos.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${STARFORGEOS_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${STARFORGEOS_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="starforgeos"
PACKAGE_NAME="${STARFORGEOS_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (STARFORGEOS_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${STARFORGEOS_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${STARFORGEOS_INSTALL_LATEST_OUT:-}"
fi
