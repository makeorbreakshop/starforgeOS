...
- You can run the CLI (`starforgeos`) on the gateway machine (or via SSH).
...
- Set `gateway.bind: "tailnet"` in `~/.starforgeos/starforgeos.json` on the gateway host.
...
1. Set up a DNS-SD zone (example `starforgeos.internal.`) on the gateway host and publish `_openclaw-gw._tcp` records.
...
1. Create `~/.starforgeos/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
starforge nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18793/__openclaw__/canvas/"}'
```
