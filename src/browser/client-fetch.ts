...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  const hint = isAbsoluteHttp(url)
    ...
    : `Start (or restart) the StarforgeOS gateway (StarforgeOS.app menubar, or \`${formatCliCommand("starforge gateway")}\`) and try again.`;
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the StarforgeOS browser control service (timed out after ${timeoutMs}ms). ${hint}`,
    );
  }
  return new Error(`Can't reach the StarforgeOS browser control service. ${hint} (${msg})`);
}
