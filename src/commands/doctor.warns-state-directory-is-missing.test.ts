...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "starforge-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.STARFORGEOS_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when STARFORGEOS_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/starforge.json",
      exists: true,
      raw: "{}",
    ...
    process.env.STARFORGEOS_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
