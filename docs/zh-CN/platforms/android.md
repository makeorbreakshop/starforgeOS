...
- 你可以在 Gateway 网关机器上运行 CLI（`starforgeos`）（或通过 SSH）。
...
- 在 Gateway 网关主机的 `~/.starforgeos/starforgeos.json` 中设置 `gateway.bind: "tailnet"`。
...
1. 在 Gateway 网关主机上设置 DNS-SD 区域（示例 `starforgeos.internal.`）并发布 `_openclaw-gw._tcp` 记录。
...
1. 在 Gateway 网关主机上创建 `~/.starforgeos/workspace/canvas/index.html`。
