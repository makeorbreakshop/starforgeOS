...
StarforgeOS integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- StarforgeOS owns lifecycle when `channels.signal.autoStart=true`.
...
- StarforgeOS spawns `imsg rpc` as a child process (legacy iMessage integration).
