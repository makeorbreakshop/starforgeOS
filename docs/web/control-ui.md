...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/starforgeos`)
...
If the page fails to load, start the Gateway first: `starforge gateway`.
...
you revoke it with `starforge devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.starforgeos/starforgeos.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. StarforgeOS
...
StarforgeOS **blocks** Control UI connections without device identity.
