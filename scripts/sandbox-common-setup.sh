...
BASE_IMAGE="${BASE_IMAGE:-starforgeos-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-starforgeos-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=starforgeos.sandbox=1)
NOTE
