...
IMAGE_NAME="starforgeos-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="starforgeos-net-e2e-$$"
GW_NAME="starforgeos-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "STARFORGEOS_GATEWAY_TOKEN=$TOKEN" \
	  -e "STARFORGEOS_SKIP_CHANNELS=1" \
	  -e "STARFORGEOS_SKIP_GMAIL_WATCHER=1" \
	  -e "STARFORGEOS_SKIP_CRON=1" \
	  -e "STARFORGEOS_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
