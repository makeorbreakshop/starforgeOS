...
function candidateBinDirs(opts: EnsureOpenClawPathOpts): string[] {
  ...
  // Bundled macOS app: `starforge` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "starforge");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/starforge` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "starforge"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `starforge` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpenClawCliOnPath(opts: EnsureOpenClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.STARFORGEOS_PATH_BOOTSTRAPPED)) {
    return;
  }
