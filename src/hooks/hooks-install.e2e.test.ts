...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "starforge-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevStateDir = process.env.STARFORGEOS_STATE_DIR;
    prevBundledDir = process.env.STARFORGEOS_BUNDLED_HOOKS_DIR;
    process.env.STARFORGEOS_STATE_DIR = path.join(baseDir, "state");
    process.env.STARFORGEOS_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      ...
      process.env.STARFORGEOS_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
  ...
  it("installs a hook pack and triggers the handler", async () => {
      ...
      JSON.stringify(
        {
          ...
          version: "0.0.0",
          starforge: { hooks: ["./hooks/hello-hook"] },
        },
        null,
        2,
      ),
    ...
    await fs.writeFile(
      ...
      [
        ...
        'metadata: {"starforge":{"events":["command:new"]}}',
        "---",
        "",
        "# Hello Hook",
        "",
