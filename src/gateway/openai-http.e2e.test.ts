    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "starforge", messages: [{ role: "user", content: "hi" }] },
          { "x-starforge-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-starforge-agent-id": "beta",
            "x-starforge-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
