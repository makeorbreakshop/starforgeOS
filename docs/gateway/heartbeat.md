...
- During heartbeat runs, StarforgeOS treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `starforge sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, StarforgeOS skips the heartbeat run entirely (no model call).
...
```bash
starforge system event --text "Check for urgent follow-ups" --mode now
```
