...
Tlon is a decentralized messenger built on Urbit. StarforgeOS connects to your Urbit ship and can
...
```bash
starforge plugins install @openclaw/tlon
```
...
Use these with `starforge message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, StarforgeOS replies in-thread.
