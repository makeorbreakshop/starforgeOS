import type { PluginRuntime } from "starforgeos/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.STARFORGEOS_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "starforgeos-nostr-"));
  process.env.STARFORGEOS_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.STARFORGEOS_STATE_DIR?.trim() || env.STARFORGEOS_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".starforgeos");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.STARFORGEOS_STATE_DIR = previous;
    }
