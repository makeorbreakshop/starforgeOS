...
你可以在 `~/.starforgeos/starforgeos.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/starforgeos.log"
  }
}
```
...
```bash
starforge channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.starforgeos/starforgeos.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/starforgeos/starforgeos-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- StarforgeOS 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
STARFORGEOS_DIAGNOSTICS=telegram.http,telegram.payload
```
...
```json
{
  ...
  "diagnostics": {
    ...
    "otel": {
      ...
      "serviceName": "starforgeos-gateway",
      "traces": true,
      "metrics": true,
      "logs": true,
      "sampleRate": 0.2,
...
- 你也可以使用 `starforge plugins enable diagnostics-otel` 启用插件。
...
- `starforgeos.tokens`（计数器，属性：`starforgeos.token`、`starforgeos.channel`、`starforgeos.provider`、`starforgeos.model`）
- `starforgeos.cost.usd`（计数器，属性：`starforgeos.channel`、`starforgeos.provider`、`starforgeos.model`）
- `starforgeos.run.duration_ms`（直方图，属性：`starforgeos.channel`、`starforgeos.provider`、`starforgeos.model`）
- `starforgeos.context.tokens`（直方图，属性：`starforgeos.context`、`starforgeos.channel`、`starforgeos.provider`、`starforgeos.model`）

消息流：

- `starforgeos.webhook.received`（计数器，属性：`starforgeos.channel`、`starforgeos.webhook`）
- `starforgeos.webhook.error`（计数器，属性：`starforgeos.channel`、`starforgeos.webhook`）
- `starforgeos.webhook.duration_ms`（直方图，属性：`starforgeos.channel`、`starforgeos.webhook`）
- `starforgeos.message.queued`（计数器，属性：`starforgeos.channel`、`starforgeos.source`）
- `starforgeos.message.processed`（计数器，属性：`starforgeos.channel`、`starforgeos.outcome`）
- `starforgeos.message.duration_ms`（直方图，属性：`starforgeos.channel`、`starforgeos.outcome`）

队列 + 会话：

- `starforgeos.queue.lane.enqueue`（计数器，属性：`starforgeos.lane`）
- `starforgeos.queue.lane.dequeue`（计数器，属性：`starforgeos.lane`）
- `starforgeos.queue.depth`（直方图，属性：`starforgeos.lane` 或 `starforgeos.channel=heartbeat`）
- `starforgeos.queue.wait_ms`（直方图，属性：`starforgeos.lane`）
- `starforgeos.session.state`（计数器，属性：`starforgeos.state`、`starforgeos.reason`）
- `starforgeos.session.stuck`（计数器，属性：`starforgeos.state`）
- `starforgeos.session.stuck_age_ms`（直方图，属性：`starforgeos.state`）
- `starforgeos.run.attempt`（计数器，属性：`starforgeos.attempt`）

### 导出的 span（名称 + 关键属性）

- `starforgeos.model.usage`
  - `starforgeos.channel`、`starforgeos.provider`、`starforgeos.model`
  - `starforgeos.sessionKey`、`starforgeos.sessionId`
  - `starforgeos.tokens.*`（input/output/cache_read/cache_write/total）
- `starforgeos.webhook.processed`
  - `starforgeos.channel`、`starforgeos.webhook`、`starforgeos.chatId`
- `starforgeos.webhook.error`
  - `starforgeos.channel`、`starforgeos.webhook`、`starforgeos.chatId`、`starforgeos.error`
- `starforgeos.message.processed`
  - `starforgeos.channel`、`starforgeos.outcome`、`starforgeos.chatId`、`starforgeos.messageId`、`starforgeos.sessionKey`、`starforgeos.sessionId`、`starforgeos.reason`
- `starforgeos.session.stuck`
  - `starforgeos.state`、`starforgeos.ageMs`、`starforgeos.queueDepth`、`starforgeos.sessionKey`、`starforgeos.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `starforge doctor`。
