...
IMAGE_NAME="starforgeos-plugins-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  home_dir=$(mktemp -d "/tmp/starforgeos-plugins-e2e.XXXXXX")
  export HOME="$home_dir"
  mkdir -p "$HOME/.starforgeos/extensions/demo-plugin"

  cat > "$HOME/.starforgeos/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.starforgeos/extensions/demo-plugin/starforgeos.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/starforgeos-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@openclaw/demo-plugin-tgz",
  "version": "0.0.1",
  "starforgeos": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$pack_dir/package/starforgeos.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/starforgeos-plugin-dir.XXXXXX")"
...
JS
  cat > "$dir_plugin/starforgeos.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/starforgeos-plugin-filepack.XXXXXX")"
...
JS
  cat > "$file_pack_dir/package/starforgeos.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
