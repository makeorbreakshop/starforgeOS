...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.STARFORGEOS_CONFIG_PATH;
  const prevDisableCache = process.env.STARFORGEOS_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "starforge-canvas-auth-test-"));
  const configPath = path.join(dir, "starforge.json");

  process.env.STARFORGEOS_CONFIG_PATH = configPath;
  process.env.STARFORGEOS_DISABLE_CONFIG_CACHE = "1";
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.STARFORGEOS_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.STARFORGEOS_DISABLE_CONFIG_CACHE;
    } else {
      process.env.STARFORGEOS_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
