...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenClawAgentDir: vi.fn().mockReturnValue("/tmp/starforge-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/starforge-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/starforge-agent/auth-profiles.json"),
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/starforge-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/starforge-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/starforge-agent-custom");
