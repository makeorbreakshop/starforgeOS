...
IMAGE_NAME="starforgeos-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/starforgeos-bin:$PATH"
  mkdir -p /tmp/starforgeos-bin
  cat > /tmp/starforgeos-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/starforgeos-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/starforgeos-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/starforgeos-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/starforgeos-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/.starforgeos/workspace"
    config_path="$HOME/.starforgeos/starforgeos.json"
    sessions_dir="$HOME/.starforgeos/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.starforgeos/starforgeos.json" <<'"'"'JSON'"'"'
{
