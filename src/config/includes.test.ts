...
const ETC_STARFORGEOS_DIR = path.join(ROOT_DIR, "etc", "starforge");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "starforge.json");
...
function etcOpenClawPath(...parts: string[]) {
  return path.join(ETC_STARFORGEOS_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "starforge.json"))).toEqual({
      shared: true,
    });
  });
});
