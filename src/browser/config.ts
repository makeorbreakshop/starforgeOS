...
import {
  ...
  DEFAULT_STARFORGEOS_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "starforge" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_STARFORGEOS_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "starforge" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_STARFORGEOS_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_STARFORGEOS_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an StarforgeOS browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for starforge/starforge CDP).
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_STARFORGEOS_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "starforge";
