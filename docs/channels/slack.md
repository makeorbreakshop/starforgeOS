...
3. Set tokens for StarforgeOS and start the gateway.
...
7. Slash Commands → create `/starforgeos` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
### StarforgeOS config (Socket mode)
...
StarforgeOS can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, StarforgeOS falls back to the user token.
...
```json
{
  "display_information": {
    "name": "StarforgeOS",
    "description": "Slack connector for StarforgeOS"
  },
  "features": {
    "bot_user": {
      "display_name": "StarforgeOS",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/starforgeos",
        "description": "Send a message to StarforgeOS",
        "should_escape": false
      }
    ]
  },
...
By default, StarforgeOS replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, StarforgeOS infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `starforge pairing approve slack <code>`.
...
- On startup, StarforgeOS resolves channel/user names in allowlists to IDs (when tokens allow)
