...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "STARFORGEOS_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("STARFORGEOS_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv STARFORGEOS_GATEWAY_TOKEN");
    expect(message).not.toContain("STARFORGEOS_GATEWAY_PASSWORD");
  });
