...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `starforge --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.STARFORGEOS_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.STARFORGEOS_GATEWAY_BIND ??
...
void main().catch((err) => {
  console.error(
    "[starforge] Gateway daemon failed:",
