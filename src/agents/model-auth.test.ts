...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "starforge-oauth-"));

    try {
      process.env.STARFORGEOS_STATE_DIR = tempDir;
      process.env.STARFORGEOS_AGENT_DIR = path.join(tempDir, "agent");
      ...
      const store = ensureAuthProfileStore(process.env.STARFORGEOS_AGENT_DIR, {
