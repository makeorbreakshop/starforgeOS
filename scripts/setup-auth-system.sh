#!/bin/bash
# Setup StarforgeOS Auth Management System
...
echo "=== StarforgeOS Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'starforgeos-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  StarforgeOS message: Send warning via StarforgeOS itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/starforgeos-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/starforgeos-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/starforgeos-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now starforgeos-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status starforgeos-auth-monitor.timer"
echo ""
echo "Quick commands:"
