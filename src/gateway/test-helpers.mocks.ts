...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `starforge-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.STARFORGEOS_CONFIG_PATH = path.join(root, "starforge.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "starforge.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "starforge-gateway-test"),
        ...fileDefaults,
