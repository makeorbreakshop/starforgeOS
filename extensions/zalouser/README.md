...
StarforgeOS extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
starforge channels login --channel zalouser
...
```bash
starforge message send --channel zalouser --target <threadId> --message "Hello from StarforgeOS!"
```
...
```bash
starforge channels login --channel zalouser              # Login via QR
starforge channels login --channel zalouser --account work
starforge channels status --probe
starforge channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
starforge directory self --channel zalouser
starforge directory peers list --channel zalouser --query "name"
starforge directory groups list --channel zalouser --query "work"
starforge directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
starforge message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
starforge message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `starforge channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
starforge channels login --channel zalouser --account work
starforge message send --channel zalouser --account work --target <id> --message "Hello"
