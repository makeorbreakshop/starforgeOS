---
summary: "Use Venice AI privacy-focused models in StarforgeOS"
read_when:
  - You want privacy-focused inference in StarforgeOS
...
```bash
starforge onboard --auth-choice venice-api-key
```
...
```bash
starforge chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, StarforgeOS shows all available Venice models. Pick based on your needs:
...
```bash
starforge models set venice/claude-opus-45
starforge models set venice/llama-3.3-70b
```

List all available models:

```bash
starforge models list | grep venice
```

## Configure via `starforge configure`

1. Run `starforge configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
StarforgeOS automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
starforge chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
starforge chat --model venice/claude-opus-45

# Use uncensored model
starforge chat --model venice/venice-uncensored

# Use vision model with image
starforge chat --model venice/qwen3-vl-235b-a22b

# Use coding model
starforge chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `starforge models list` to see currently available models. Some models may be temporarily offline.
