# StarforgeOS ACP Bridge

This document describes how the StarforgeOS ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`starforge acp` exposes an ACP agent over stdio and forwards prompts to a running
StarforgeOS Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a StarforgeOS Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `starforge acp` over stdio.

Example config:

```bash
starforge config set gateway.remote.url wss://gateway-host:18789
starforge config set gateway.remote.token <token>
```

Example run:

```bash
starforge acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
starforge acp --session agent:main:main
starforge acp --session agent:design:main
starforge acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “StarforgeOS ACP” to start a thread.

## Execution Model

- ACP client spawns `starforge acp` and speaks ACP messages over stdio.
...
```bash
starforge acp --session agent:main:main
starforge acp --session-label "support inbox"
starforge acp --reset-session
```
...
`starforge acp` resolves the Gateway URL and auth from CLI flags or config:
