...
```bash
starforge tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `starforge tui --deliver`
...
- Check the Gateway logs: `starforge logs --follow`.
- Confirm the agent can run: `starforge status` and `starforge models status`.
...
- No agents in picker: check `starforge agents list` and your routing config.
