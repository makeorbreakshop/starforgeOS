...
export async function noteSecurityWarnings(cfg: OpenClawConfig) {
  ...
  const auditHint = `- Run: ${formatCliCommand("starforge security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("starforge configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("starforge config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("starforge doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("starforge config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
