...
read_when:
  - You need detailed behavior for starforge onboard
...
This page is the full reference for `starforge onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.starforgeos/starforgeos.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `starforge doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `starforge pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `starforge health`.
    - `starforge status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.starforgeos/.env` so launchd can read it.
...
- OAuth credentials: `~/.starforgeos/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.starforgeos/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`starforge onboard`](/cli/onboard)
