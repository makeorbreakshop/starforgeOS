...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.STARFORGEOS_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*starforge-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "starforge";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "starforge");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "starforge-gw");
  ...
  records.push(`_starforge-gw._tcp IN PTR ${instanceLabel}._starforge-gw._tcp`);
  records.push(`${instanceLabel}._starforge-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._starforge-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; starforge-content-hash: ${contentHash}\n${contentBody}`;
}
