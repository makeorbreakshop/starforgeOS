...
const SEARCH_TOOL = "https://docs.starforge.ai/mcp.SearchOpenClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.starforge.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('starforge docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.starforge.ai/");
      runtime.log(`Search: ${formatCliCommand('starforge docs "your query"')}`);
    }
    return;
  }
