...
service via the external `starforgeos` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`STARFORGEOS_PROFILE`; legacy `com.starforgeos.*` is supported).
