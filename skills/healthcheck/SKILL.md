---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for StarforgeOS deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, StarforgeOS cron scheduling for periodic checks, or version status checks on a machine running StarforgeOS (laptop, workstation, Pi, VPS).
---

# StarforgeOS Host Hardening

## Overview

Assess and harden the host running StarforgeOS, then align it to a user-defined risk tolerance without breaking access. Use StarforgeOS security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim StarforgeOS changes the host firewall, SSH, or OS updates; it does not.
...
5. StarforgeOS gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if StarforgeOS can access sensitive data.
...
### 2) Run StarforgeOS security audits (read-only)

As part of the default read-only checks, run `starforge security audit --deep`. Only offer alternatives if the user requests them:

1. `starforge security audit` (faster, non-probing)
2. `starforge security audit --json` (structured output)

Offer to apply StarforgeOS safe defaults (numbered):

1. `starforge security audit --fix`

Be explicit that `--fix` only tightens StarforgeOS defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check StarforgeOS version/update status (read-only)

As part of the default read-only checks, run `starforge update status`.
...
- Credential hygiene notes (location of StarforgeOS creds, prefer disk encryption)
...
- StarforgeOS security audit (re-run)
...
After StarforgeOS install or first hardening pass, run at least one baseline audit and version check:

- `starforge security audit`
- `starforge security audit --deep`
- `starforge update status`

Ongoing monitoring is recommended. Use the StarforgeOS cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `starforge cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `starforge update status`
...
Before creating, `starforge cron list` and match on exact `name`. If found, `starforge cron edit <id> ...`.
If not found, `starforge cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `starforge update status` (preferred for source checkouts and channels)
2. `npm view starforge version` (published npm version)

## StarforgeOS command accuracy

Use only supported commands and flags:

- `starforge security audit [--deep] [--fix] [--json]`
- `starforge status` / `starforge status --deep`
- `starforge health --json`
- `starforge update status`
- `starforge cron add|list|runs|run`

Do not invent CLI flags or imply StarforgeOS enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by StarforgeOS compaction:
