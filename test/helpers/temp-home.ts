...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("STARFORGEOS_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.STARFORGEOS_STATE_DIR = path.join(base, ".starforgeos");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "starforgeos-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".starforgeos", "agents", "main", "sessions"), {
    recursive: true,
  });
  if (opts.env) {
