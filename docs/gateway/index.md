...
- Replaces the legacy `gateway` command. CLI entry point: `starforge gateway`.
...
starforge gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.starforgeos/starforgeos.json` (or `STARFORGEOS_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__openclaw__/canvas/` from `~/.starforgeos/workspace/canvas`. Disable with `canvasHost.enabled=false` or `STARFORGEOS_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `STARFORGEOS_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `STARFORGEOS_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.starforgeos.*` may still exist)
- Linux: `starforgeos-gateway-<profile>.service`
- Windows: `StarforgeOS Gateway (<profile>)`

Install metadata is embedded in the service config:

- `STARFORGEOS_SERVICE_MARKER=starforgeos`
- `STARFORGEOS_SERVICE_KIND=gateway`
- `STARFORGEOS_SERVICE_VERSION=<version>`
...
```bash
starforge --dev setup
starforge --dev gateway --allow-unconfigured
# then target the dev instance:
starforge --dev status
starforge --dev health
```

Defaults (can be overridden via env/flags/config):

- `STARFORGEOS_STATE_DIR=~/.starforgeos-dev`
- `STARFORGEOS_CONFIG_PATH=~/.starforgeos-dev/starforgeos.json`
- `STARFORGEOS_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.starforgeos/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `STARFORGEOS_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `STARFORGEOS_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `STARFORGEOS_CONFIG_PATH`
- unique `STARFORGEOS_STATE_DIR`
...
```bash
starforge --profile main gateway install
starforge --profile rescue gateway install
```

Example:

```bash
STARFORGEOS_CONFIG_PATH=~/.starforgeos/a.json STARFORGEOS_STATE_DIR=~/.starforgeos-a starforge gateway --port 19001
STARFORGEOS_CONFIG_PATH=~/.starforgeos/b.json STARFORGEOS_STATE_DIR=~/.starforgeos-b starforge gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `starforge health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `starforgeos`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `starforge gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.starforgeos.*` is cleaned up).
  - `starforge doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are StarforgeOS profile services.
  ...
  - Cleanup: `starforge gateway uninstall` (current service) and `starforge doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `starforge gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- StarforgeOS.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.starforgeos.*` labels still unload cleanly).
- To stop it cleanly, use `starforge gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `starforge gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `starforge gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

StarforgeOS installs a **systemd user service** by default on Linux/WSL2. We
...
`starforge gateway install` writes the user unit. `starforge doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/starforgeos-gateway[-<profile>].service`:

```
[Unit]
Description=StarforgeOS Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/starforge gateway --port 18789
Restart=always
RestartSec=5
Environment=STARFORGEOS_GATEWAY_TOKEN=
...
```
systemctl --user enable --now starforgeos-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/starforgeos-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now starforgeos-gateway[-<profile>].service
```
...
- `starforge gateway health|status` — request health/status over the Gateway WS.
- `starforge message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `starforge agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `starforge gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `starforge gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `starforge gateway` and the legacy TCP control port.
