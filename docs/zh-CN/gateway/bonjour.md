...
StarforgeOS 使用 Bonjour（mDNS / DNS‑SD）作为**仅限局域网的便捷方式**来发现
...
2. 在专用区域下发布 `_openclaw-gw._tcp` 的 DNS‑SD 记录
   （示例：`starforgeos.internal.`）。
3. 配置 Tailscale **分割 DNS**，使你选择的域名通过该
   DNS 服务器为客户端（包括 iOS）解析。

StarforgeOS 支持任何发现域名；`starforgeos.internal.` 只是一个示例。
...
- 从 `~/.starforgeos/dns/<domain>.db` 提供你选择的域名服务（示例：`starforgeos.internal.`）
...
- 在 `~/.starforgeos/starforgeos.json` 中设置 `gateway.bind: "tailnet"`。
...
- `transport=gateway`
- `cliPath=<path>`（可选；可运行的 `starforgeos` 入口点的绝对路径）
...
- `STARFORGEOS_DISABLE_BONJOUR=1` 禁用广播（旧版：`STARFORGEOS_DISABLE_BONJOUR`）。
- `~/.starforgeos/starforgeos.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
- `STARFORGEOS_SSH_PORT` 覆盖 TXT 中广播的 SSH 端口（旧版：`STARFORGEOS_SSH_PORT`）。
- `STARFORGEOS_TAILNET_DNS` 在 TXT 中发布 MagicDNS 提示（旧版：`STARFORGEOS_TAILNET_DNS`）。
- `STARFORGEOS_CLI_PATH` 覆盖广播的 CLI 路径（旧版：`STARFORGEOS_CLI_PATH`）。
