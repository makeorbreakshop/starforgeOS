...
<title>StarforgeOS Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.starforgeCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.starforgeCanvasA2UIAction &&
        typeof window.starforgeCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.starforgeSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("starforge:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android StarforgeOS node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.starforgeSendUserAction === "function"
        ? window.starforgeSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.STARFORGEOS_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.STARFORGEOS_SKIP_CANVAS_HOST)) {
    return true;
...
function resolveDefaultCanvasRoot(): string {
  const candidates = [path.join(os.homedir(), ".starforge", "canvas")];
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>StarforgeOS Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
