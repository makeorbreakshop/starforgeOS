...
metadata:
  { "starforgeos": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---

# tmux Skill (StarforgeOS)
...
```bash
SOCKET_DIR="${STARFORGEOS_TMUX_SOCKET_DIR:-${CLAWDBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/starforgeos-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/starforgeos.sock"
SESSION=starforgeos-python
...
- Use `STARFORGEOS_TMUX_SOCKET_DIR` (legacy `CLAWDBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$STARFORGEOS_TMUX_SOCKET_DIR/starforgeos.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `STARFORGEOS_TMUX_SOCKET_DIR`).
