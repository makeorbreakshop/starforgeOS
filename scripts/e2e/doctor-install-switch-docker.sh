...
IMAGE_NAME="starforgeos-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/starforgeos-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/starforgeos-bin/systemctl

  cat > /tmp/starforgeos-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/starforgeos-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/starforgeos"
  npm_entry="/tmp/npm-prefix/lib/node_modules/starforgeos/dist/index.js"
  git_entry="/app/dist/index.js"
  git_cli="/app/starforgeos.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/starforgeos-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/starforgeos-gateway.service"
    if [ ! -f "$unit_path" ]; then
