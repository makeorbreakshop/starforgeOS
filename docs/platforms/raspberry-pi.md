---
summary: "StarforgeOS on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up StarforgeOS on a Raspberry Pi
  - Running StarforgeOS on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# StarforgeOS on Raspberry Pi

## Goal

Run a persistent, always-on StarforgeOS Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most StarforgeOS features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled starforgeos

# Enable if not
sudo systemctl enable starforgeos

# Start on boot
sudo systemctl start starforgeos
```
...
```bash
# Check logs
journalctl -u starforge --no-pager -n 100

# Common fix: rebuild
cd ~/starforge  # if using hackable install
npm run build
sudo systemctl restart starforgeos
```
