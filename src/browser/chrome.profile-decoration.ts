...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".starforge-profile-decorated");
}
...
export function decorateOpenClawProfile(
  ...
  const desiredName = opts?.name ?? DEFAULT_STARFORGEOS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_STARFORGEOS_BROWSER_COLOR).toUpperCase();
