---
summary: "Chrome extension: let StarforgeOS drive your existing Chrome tab"
read_when:
...
The StarforgeOS Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate starforgeos-managed Chrome profile.
...
StarforgeOS then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the StarforgeOS release (npm package) as static files. There is no separate “build” step.

After upgrading StarforgeOS:

- Re-run `starforge browser extension install` to refresh the installed files under your StarforgeOS state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

StarforgeOS ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want StarforgeOS to control.
...
- `ON`: attached; StarforgeOS can drive that tab.
...
Debugging: `starforge sandbox explain`
...
`starforge browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `starforge browser extension install` first to copy the extension to a stable location under your StarforgeOS state directory.
...
- **This is not isolated** like the dedicated starforgeos-managed profile.
