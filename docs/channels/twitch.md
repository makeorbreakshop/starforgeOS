...
read_when:
  - Setting up Twitch chat integration for StarforgeOS
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. StarforgeOS connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
starforge plugins install @openclaw/twitch
```
...
4. Configure the token:
   - Env: `STARFORGEOS_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "starforgeos", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use STARFORGEOS_TWITCH_ACCESS_TOKEN env var)
...
```bash
STARFORGEOS_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
