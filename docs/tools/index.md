---
summary: "Agent tool surface for StarforgeOS (browser, canvas, nodes, message, cron) replacing legacy `starforgeos-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `starforgeos-*` skills
title: "Tools"
---

# Tools (StarforgeOS)

StarforgeOS exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `starforgeos-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `starforgeos.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, StarforgeOS logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`starforge node run`).
...
- Requires a Brave API key (recommended: `starforge configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated StarforgeOS-managed browser.
...
- Quick smoke: `starforge nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `starforge gateway` restart in-place)
