...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "starforge.sandbox=1");
  args.push("--label", `starforge.sessionKey=${params.scopeKey}`);
  args.push("--label", `starforge.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `starforge.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("starforge.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`starforge sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`starforge sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("starforge sandbox recreate --all");
}
