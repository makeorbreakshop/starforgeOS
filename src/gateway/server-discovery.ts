...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/starforge/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (StarforgeOS)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.STARFORGEOS_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "starforge");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.STARFORGEOS_TAILNET_DNS?.trim();
