...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `starforge sandbox explain`.
...
Legacy `agent.*` configs are migrated by `starforge doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=starforgeos-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${STARFORGEOS_STATE_DIR:-$HOME/.starforgeos}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
