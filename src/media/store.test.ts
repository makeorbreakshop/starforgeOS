...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "STARFORGEOS_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "starforge-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.STARFORGEOS_STATE_DIR = path.join(home, ".starforge");
    ...
    await fs.mkdir(path.join(home, ".starforge"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.starforge${path.sep}media`);
