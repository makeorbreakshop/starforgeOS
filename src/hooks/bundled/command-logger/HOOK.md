...
homepage: https://docs.starforge.ai/hooks#command-logger
metadata:
  {
    "starforge":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.starforge/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.starforge/logs/commands.log ~/.starforge/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/starforge`:
   ```
   /home/username/.starforge/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.starforge/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.starforge/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.starforge/logs/commands.log | jq .
```
