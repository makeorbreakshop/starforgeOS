---
summary: "All configuration options for ~/.starforgeos/starforgeos.json with examples"
read_when:
...
StarforgeOS reads an optional **JSON5** config from `~/.starforgeos/starforgeos.json` (comments + trailing commas allowed).

If the file is missing, StarforgeOS uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.starforgeos/workspace`). You usually only need a config to:
...
StarforgeOS only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `starforge doctor`, `starforge logs`, `starforge health`, `starforge status`, `starforge service`, `starforge help`).
- Run `starforge doctor` to see the exact issues.
- Run `starforge doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `starforge config set`. Keep a backup of `~/.starforgeos/starforgeos.json`.
...
```bash
starforge gateway call config.get --params '{}' # capture payload.hash
starforge gateway call config.apply --params '{
...
```bash
starforge gateway call config.get --params '{}' # capture payload.hash
starforge gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.starforgeos/workspace" },
    list: [
      {
        id: "main",
...
```json5
// ~/.starforgeos/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.starforgeos/workspace" }],
}
```
...
{ "$include": "/etc/starforgeos/base.json5" } // absolute
...
```json5
// ~/.starforgeos/clients/mueller/agents.json5
[
...
```json5
// ~/.starforgeos/clients/mueller/broadcast.json5
{
...
StarforgeOS reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, StarforgeOS runs your login shell and imports only the missing expected keys (never overrides).
...
- `STARFORGEOS_LOAD_SHELL_ENV=1`
- `STARFORGEOS_SHELL_ENV_TIMEOUT_MS=15000`
...
```json5
{
  ...
  gateway: {
    auth: {
      token: "${STARFORGEOS_GATEWAY_TOKEN}",
    },
  },
}
```
...
StarforgeOS stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.starforgeos/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.starforgeos/credentials/oauth.json` (or `$STARFORGEOS_STATE_DIR/credentials/oauth.json`)
...
- `~/.starforgeos/agent/*` (migrated by `starforge doctor` into `~/.starforgeos/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `STARFORGEOS_OAUTH_DIR`
- Agent dir (default agent root override): `STARFORGEOS_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, StarforgeOS imports `oauth.json` entries into `auth-profiles.json`.
...
If set, StarforgeOS derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/starforgeos/starforgeos-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/starforgeos/starforgeos.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.starforgeos/credentials/whatsapp/biz
          // authDir: "~/.starforgeos/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `starforge doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@openclaw", "starforgeos"] } }],
  },
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.starforgeos/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.starforgeos/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.starforgeos/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.starforgeos/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.starforgeos/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.starforgeos/workspace-home" },
      { id: "work", workspace: "~/.starforgeos/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `starforgeos.json`).
...
StarforgeOS starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["starforgeos-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
...
StarforgeOS starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `starforge plugins install @openclaw/mattermost` (or `./extensions/mattermost` from a git checkout).
...
StarforgeOS starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
StarforgeOS spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, StarforgeOS
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/starforgeos" } },
}
```
...
When a file exceeds this limit, StarforgeOS logs a warning and injects a truncated
...
message envelopes). If unset, StarforgeOS uses the host timezone at runtime.
...
`[starforgeos]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[starforgeos]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[starforgeos]"`, StarforgeOS will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, StarforgeOS generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.starforgeos/settings/tts.json",
      elevenlabs: {
...
StarforgeOS also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, StarforgeOS currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `starforge configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:starforgeos`: all built-in StarforgeOS tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.starforgeos/sandboxes`
    ...
    defaults: {
      sandbox: {
        ...
        docker: {
          ...
          containerPrefix: "starforgeos-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "starforgeos-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "starforgeos-sandbox-browser:bookworm-slim",
          containerPrefix: "starforgeos-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
StarforgeOS uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.starforgeos/agents/<agentId>/agent/models.json` or by defining the same schema inside your
StarforgeOS config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, StarforgeOS writes/merges a `models.json` into
`~/.starforgeos/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. StarforgeOS uses
...
- Shortcut: `starforge onboard --auth-choice opencode-zen`.
...
Shortcut: `starforge onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `starforge onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `starforge onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `starforge onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `starforge onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `starforge onboard --auth-choice minimax-api`.
...
- Override the agent config root with `STARFORGEOS_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.starforgeos/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.starforgeos/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.starforgeos/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, StarforgeOS stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.starforgeos/skills` (workspace skills
...
from `~/.starforgeos/extensions`, `<workspace>/.starforgeos/extensions`, plus any
...
### `browser` (starforgeos-managed browser)

StarforgeOS can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for starforge and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (StarforgeOS.app menubar, or `starforge gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      starforgeos: { cdpPort: 18800, color: "#FF4500" },
...
- Examples: `"/ui"`, `"/starforgeos"`, `"/apps/starforgeos"`.
...
- When a connection comes from one of these IPs, StarforgeOS uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `starforge gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `STARFORGEOS_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `STARFORGEOS_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. StarforgeOS
...
- StarforgeOS.app watches `~/.starforgeos/starforgeos.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.starforgeos/starforgeos.json` (or `STARFORGEOS_CONFIG_PATH`) and applies changes automatically.
...
- `~/.starforgeos/starforgeos.json` (or `STARFORGEOS_CONFIG_PATH`)
...
- `STARFORGEOS_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `starforge --dev …` → uses `~/.starforgeos-dev` + shifts ports from base `19001`
- `starforge --profile <name> …` → uses `~/.starforgeos-<name>` (port via config/env/flags)
...
```bash
STARFORGEOS_CONFIG_PATH=~/.starforgeos/a.json \
STARFORGEOS_STATE_DIR=~/.starforgeos-a \
starforge gateway --port 19001
```
...
Gmail helper config (used by `starforge webhooks gmail setup` / `run`):
...
- Set `STARFORGEOS_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, StarforgeOS defaults `serve.path` to `/` so
...
Default root: `~/.starforgeos/workspace/canvas`  
Default port: `18793` (chosen to avoid the starforge browser CDP port `18792`)  
...
{
  canvasHost: {
    root: "~/.starforgeos/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
- env: `STARFORGEOS_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.starforgeos/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.starforgeos/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.starforgeos/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
- Hostname: defaults to `starforgeos` (advertises `starforgeos.local`). Override with `STARFORGEOS_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_openclaw-gw._tcp` under `~/.starforgeos/dns/` using the configured discovery domain (example: `starforgeos.internal.`).
