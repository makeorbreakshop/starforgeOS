...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.STARFORGEOS_SKIP_CHANNELS;
    prevSkipGmail = process.env.STARFORGEOS_SKIP_GMAIL_WATCHER;
    process.env.STARFORGEOS_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.STARFORGEOS_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.STARFORGEOS_SKIP_GMAIL_WATCHER;
    } else {
      process.env.STARFORGEOS_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
