...
metadata: { "starforgeos": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == StarforgeOS `sessions_spawn`
- **File I/O** == StarforgeOS `read`/`write`
- **Remote fetch** == StarforgeOS `web_fetch` (or `exec` with curl when POST is required)
