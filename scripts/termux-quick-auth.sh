...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/starforgeos/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "StarforgeOS Re-Auth" -c "After getting key, run: ssh $SERVER '~/starforgeos/scripts/mobile-reauth.sh'" --id starforgeos-auth
        ;;
    *)
