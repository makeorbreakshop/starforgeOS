...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "starforge-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "starforge-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "starforge-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "starforge-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.starforge when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "starforge-config-dir-"));
    try {
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/starforge")).toBe(path.resolve(os.homedir(), "starforge"));
  });
