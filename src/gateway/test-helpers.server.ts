...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.STARFORGEOS_STATE_DIR;
  previousConfigPath = process.env.STARFORGEOS_CONFIG_PATH;
  previousSkipBrowserControl = process.env.STARFORGEOS_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.STARFORGEOS_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.STARFORGEOS_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.STARFORGEOS_BUNDLED_PLUGINS_DIR;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "starforge-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.STARFORGEOS_STATE_DIR = path.join(tempHome, ".starforge");
  delete process.env.STARFORGEOS_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.STARFORGEOS_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.STARFORGEOS_SKIP_GMAIL_WATCHER = "1";
  process.env.STARFORGEOS_SKIP_CANVAS_HOST = "1";
  process.env.STARFORGEOS_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "starforge-test-no-bundled-extensions")
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "starforge-test-"))
    : path.join(tempHome, ".starforge-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.STARFORGEOS_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.STARFORGEOS_CONFIG_PATH;
    } else {
      process.env.STARFORGEOS_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.STARFORGEOS_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.STARFORGEOS_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.STARFORGEOS_SKIP_GMAIL_WATCHER;
    } else {
      process.env.STARFORGEOS_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.STARFORGEOS_SKIP_CANVAS_HOST;
    } else {
      process.env.STARFORGEOS_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
...
export async function startServerWithClient(
  ...
  const prev = process.env.STARFORGEOS_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.STARFORGEOS_GATEWAY_TOKEN = fallbackToken;
  }
