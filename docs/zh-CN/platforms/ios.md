...
- 网络路径：
  ...
  - 通过单播 DNS-SD 的 Tailnet（示例域：`starforgeos.internal.`），**或**
  - 手动主机/端口（备选）。
...
如果 mDNS 被阻止，使用单播 DNS-SD 区域（选择一个域；示例：`starforgeos.internal.`）和 Tailscale 分割 DNS。
...
- 配对提示从未出现：运行 `starforge nodes pending` 并手动批准。
