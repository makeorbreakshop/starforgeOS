...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `starforgeos-e2e-${name}-`));
  const configDir = path.join(homeDir, ".starforgeos");
  await fs.mkdir(configDir, { recursive: true });
  const configPath = path.join(configDir, "starforgeos.json");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          STARFORGEOS_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        STARFORGEOS_GATEWAY_TOKEN: inst.gatewayToken,
        STARFORGEOS_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          STARFORGEOS_GATEWAY_PORT: String(gwA.port),
          STARFORGEOS_GATEWAY_TOKEN: gwA.gatewayToken,
          STARFORGEOS_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          STARFORGEOS_GATEWAY_PORT: String(gwB.port),
          STARFORGEOS_GATEWAY_TOKEN: gwB.gatewayToken,
          STARFORGEOS_GATEWAY_PASSWORD: "",
        }),
