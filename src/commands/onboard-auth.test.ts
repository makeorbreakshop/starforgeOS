...
const requireAgentDir = () => {
  const agentDir = process.env.STARFORGEOS_AGENT_DIR;
  if (!agentDir) {
    throw new Error("STARFORGEOS_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.STARFORGEOS_STATE_DIR;
  const previousAgentDir = process.env.STARFORGEOS_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.STARFORGEOS_AGENT_DIR;
    } else {
      process.env.STARFORGEOS_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.STARFORGEOS_OAUTH_DIR;
  });

  it("writes auth-profiles.json under STARFORGEOS_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "starforge-oauth-"));
    process.env.STARFORGEOS_STATE_DIR = tempStateDir;
    process.env.STARFORGEOS_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.STARFORGEOS_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to STARFORGEOS_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "starforge-minimax-"));
    process.env.STARFORGEOS_STATE_DIR = tempStateDir;
    process.env.STARFORGEOS_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.STARFORGEOS_AGENT_DIR;
