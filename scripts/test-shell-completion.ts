 ...
 * `starforge update`. Use it to verify the completion installation flow
...
\`starforge update\`, \`starforge doctor\`, and \`starforge onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# StarforgeOS Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`starforge completion --install\` later to enable.`));
    return;
  }
