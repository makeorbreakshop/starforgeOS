...
read_when:
  - Designing workspace memory (~/.starforgeos/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep StarforgeOS integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.starforgeos/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.starforgeos/workspace` as canonical human-readable memory.
...
```
~/.starforgeos/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in StarforgeOS**, but keep a separable core library.

### Why integrate into StarforgeOS?

- StarforgeOS already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `starforge memory recall "…" --k 25 --since 30d`
  - `starforge memory reflect --since 7d`
...
Pragmatic take for `~/.starforgeos/workspace`:
