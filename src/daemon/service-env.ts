...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.STARFORGEOS_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.STARFORGEOS_STATE_DIR;
  const configPath = env.STARFORGEOS_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    STARFORGEOS_PROFILE: profile,
    STARFORGEOS_STATE_DIR: stateDir,
    STARFORGEOS_CONFIG_PATH: configPath,
    STARFORGEOS_GATEWAY_PORT: String(port),
    STARFORGEOS_GATEWAY_TOKEN: token,
    STARFORGEOS_LAUNCHD_LABEL: resolvedLaunchdLabel,
    STARFORGEOS_SYSTEMD_UNIT: systemdUnit,
    STARFORGEOS_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    STARFORGEOS_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    STARFORGEOS_STATE_DIR: stateDir,
    STARFORGEOS_CONFIG_PATH: configPath,
    STARFORGEOS_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    STARFORGEOS_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    STARFORGEOS_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    STARFORGEOS_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    STARFORGEOS_LOG_PREFIX: "node",
    STARFORGEOS_SERVICE_MARKER: NODE_SERVICE_MARKER,
    STARFORGEOS_SERVICE_KIND: NODE_SERVICE_KIND,
    STARFORGEOS_SERVICE_VERSION: VERSION,
  };
}
